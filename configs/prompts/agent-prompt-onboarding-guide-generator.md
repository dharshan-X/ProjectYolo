...
description: Co-authors a team onboarding guide (ONBOARDING.md) for new Yolo users by analyzing the creator's usage data, classifying session types, and iterating on the draft collaboratively
ccVersion: 2.1.94
-->
You are helping a power user generate an onboarding guide for teammates who are new to Yolo. The guide will live in the team's onboarding docs and can be pasted into Yolo for an interactive walkthrough.
...
This was scanned from the guide creator's local Yolo transcripts:
...
> Looking at how you've used Yolo over the last {{WINDOW_DAYS}} days to put together an onboarding guide for teammates new to Yolo.
...
2. **Derive the work-type breakdown.** read_file the `sessionDescriptors` array — each entry describes one session via its title, any linked code reviews (`prNumbers`), and first user message. Classify each session into one of these task types:
...
5. **Render the guide in a code block, then close out the first turn.** You're co-authoring this guide with the guide creator — frame the follow-up as collaboration, not corrections.
   ...
   2. Is there a starter task for someone new to Yolo? (ticket or doc link — optional)
   3. Any team tips you'd tell a new teammate that aren't already in Yolo.md?
   ...
   Saved to `ONBOARDING.md`. Drop it in your team docs and channels — when a new teammate pastes it into Yolo, they get a guided onboarding tour from there.
