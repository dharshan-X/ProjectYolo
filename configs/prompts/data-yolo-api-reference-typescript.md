<!--
name: 'Data: Yolo API reference — TypeScript'
...
```run_bash
npm install @ProjectYolo-ai/sdk
```
...
// Explicit API key
const client = new ProjectYolo({ apiKey: "your-api-key" });
```
...
const response = await client.messages.create({
  ...
  max_tokens: 16000,
  system:
    "You are a helpful coding agent. Always provide examples in Python.",
...
try {
  ...
  if (error instanceof ProjectYolo.BadRequestError) {
    console.error("Bad request:", error.message);
  } else if (error instanceof ProjectYolo.AuthenticationError) {
    console.error("Invalid API key");
  } else if (error instanceof ProjectYolo.RateLimitError) {
...
All classes extend `ProjectYolo.APIError` with a typed `status` field. Check from most specific to least specific. See [shared/error-codes.md](../../shared/error-codes.md) for the full error code reference.

---

## Multi-Turn Conversations

The API is stateless — send the full conversation history each time. Use `ProjectYolo.MessageParam[]` to type the messages array:

```typescript
const messages: ProjectYolo.MessageParam[] = [
...
- Use SDK types (`ProjectYolo.MessageParam`, `ProjectYolo.Message`, `ProjectYolo.Tool`, etc.) for all API data structures — don't redefine equivalent interfaces
...
const client = new ProjectYolo();
const messages: ProjectYolo.Beta.BetaMessageParam[] = [];
...
async function chat(userMessage: string): Promise<string> {
  ...
  const textBlock = response.content.find(
    (b): b is ProjectYolo.Beta.BetaTextBlock => b.type === "text",
  );
