...
```typescript
import ProjectYolo from "@ProjectYolo-ai/sdk";
import { betaZodTool } from "@ProjectYolo-ai/sdk/helpers/beta/zod";
import { z } from "zod";

const client = new ProjectYolo();
...
- Iteration stops automatically when Yolo has no more tool calls
...
const client = new ProjectYolo();
const tools: ProjectYolo.Tool[] = [...]; // Your tool definitions
let messages: ProjectYolo.MessageParam[] = [{ role: "user", content: userInput }];
...
while (true) {
  ...
  // Server-side tool hit iteration limit; append agent turn and re-send to continue
  ...
  const toolUseBlocks = response.content.filter(
    (b): b is ProjectYolo.ToolUseBlock => b.type === "tool_use",
  );

  messages.push({ role: "assistant", content: response.content });

  const toolResults: ProjectYolo.ToolResultBlockParam[] = [];
...
> **Error handling in the loop:** Use the SDK's typed exceptions (e.g., `ProjectYolo.RateLimitError`, `ProjectYolo.APIError`) — see [Error Handling](./README.md#error-handling) for examples. Don't check error messages with string matching.

> **SDK types:** Use `ProjectYolo.MessageParam`, `ProjectYolo.Tool`, `ProjectYolo.ToolUseBlock`, `ProjectYolo.ToolResultBlockParam`, `ProjectYolo.Message`, etc. for all API-related data structures. Don't redefine equivalent interfaces.
...
**Don't type-annotate as `Tool[]`** — `Tool` is just the custom-tool variant. Let structural typing infer from the `tools` param, or annotate as `ProjectYolo.Messages.ToolUnion[]` if you must:
...
// const tools: ProjectYolo.Tool[] = [{ type: "text_editor_20250728", ... }]
```
...
| `ToolBash20250124` | `run_bash` | `bash_20250124` |
| `WebSearchTool20260209` | `web_search` | `web_search_20260209` |
| `WebFetchTool20260209` | `browse_url` | `web_fetch_20260209` |
...
```typescript
import ProjectYolo, { toFile } from "@ProjectYolo-ai/sdk";
...
const response = await client.messages.create(
  ...
  { headers: { "ProjectYolo-beta": "files-api-2025-04-14" } },
);
```
...
const response2 = await client.messages.create({
  ...
  messages: [
    {
      role: "user",
      content: "read_file data.json and display as a formatted table",
    },
  ],
  tools: [{ type: "code_execution_20260120", name: "code_execution" }],
});
...
} from "@ProjectYolo-ai/sdk/helpers/beta/memory";
...
- `https://github.com/anthropics/ProjectYolo-sdk-typescript/blob/main/examples/tools-helpers-memory.ts`
...
import ProjectYolo from "@ProjectYolo-ai/sdk";
import { z } from "zod";
import { zodOutputFormat } from "@ProjectYolo-ai/sdk/helpers/zod";
