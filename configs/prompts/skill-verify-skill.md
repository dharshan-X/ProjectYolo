...
`git diff @{u}.. > /tmp/d` then read_file it. No diff at all → say so, stop.
...
run_bash security gate's surface isn't the function's return value — it's
...
**Check `.Yolo/skills/` first — even if you already know how to
