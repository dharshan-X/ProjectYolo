...
First: check the time. `Yolo.md` has a **Catch-up hours** field under Schedule (their timezone is also there). Default is 9am–5pm Mon–Fri if unset.

Outside that window → update `lastRunAt` in `.Yolo/catch-up-state.json` and end with a single line:
...
- **Who are they?** read_file `Yolo.md` — job, focus areas, the handles that identify them in connected tools.
- **What are you tracking?** read_file `.Yolo/catch-up-state.json`:
...
If `priorities` is empty (first run), bootstrap a small list from `Yolo.md` + connected tools. Two or three things. The list refines itself over time.
...
- **agent-can-act** — You could handle it without bothering them. Failing build with an obvious fix. A small review to draft.
...
· <agent-can-act item> — I can <proposed action>. Say go.
```
...
Before ending, write back to `.Yolo/catch-up-state.json`:
