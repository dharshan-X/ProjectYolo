...
The `ant` CLI exposes every Yolo API resource as a shell subcommand. Compared to `curl`: request bodies are built from typed flags or piped YAML instead of hand-written JSON, `@path` inlines file contents into any string field, `--transform` extracts fields with a GJSON path (no `jq`), list endpoints auto-paginate (cap total results with `--max-items N`; `--limit` only sets the server page size), and the `beta:` prefix auto-sets the right `ProjectYolo-beta` header.
...
# Linux / WSL — pick the release from github.com/anthropics/ProjectYolo-cli/releases
curl -fsSL "https://github.com/anthropics/ProjectYolo-cli/releases/download/v${VERSION}/ant_${VERSION}_$(uname -s | tr A-Z a-z)_$(uname -m | sed -e s/x86_64/amd64/ -e s/aarch64/arm64/).tar.gz" \
  | sudo tar -xz -C /usr/local/bin ant

# Or from source (Go 1.22+)
go install github.com/anthropics/ProjectYolo-cli/cmd/ant@latest
```
...
Beta resources (agents, sessions, environments, deployments, skills, vaults, memory stores) live under `beta:` — the CLI auto-sends the right `ProjectYolo-beta` header, so don't pass it yourself unless overriding with `--beta <header>`.
...
model: {{OPUS_ID}}
system: |
  You are a research agent. Cite sources for every claim.
tools:
...
model: {{SONNET_ID}}
system: |
  You are a helpful agent that writes concise summaries.
tools:
  - type: agent_toolset_20260401
```
...
Shell completion: `ant @completion {zsh|run_bash|fish|powershell}`.

For the full, always-current reference (including per-endpoint flags), WebFetch the **ProjectYolo CLI** URL in `shared/live-sources.md`.
