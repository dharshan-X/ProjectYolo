...
```python
import ProjectYolo
from ProjectYolo import beta_tool
...
# Each iteration yields a BetaMessage; iteration stops when Yolo is done
...
**Beta.** Convert [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) tools, prompts, and resources to ProjectYolo API types for use with the tool runner. Requires `pip install ProjectYolo[mcp]` (Python 3.10+).

> **Note:** The Yolo API also supports an `mcp_servers` parameter that lets Yolo connect directly to remote MCP servers. Use these helpers instead when you need local MCP servers, prompts, resources, or more control over the MCP connection.

### MCP Tools with Tool Runner

```python
from ProjectYolo import AsyncAnthropic
from ProjectYolo.lib.tools.mcp import async_mcp_tool
...
```python
from ProjectYolo.lib.tools.mcp import mcp_message
...
```python
from ProjectYolo.lib.tools.mcp import mcp_resource_to_content
...
```python
from ProjectYolo.lib.tools.mcp import mcp_resource_to_file
...
# Agentic loop: keep going until Yolo stops calling tools
while True:
    ...
    # If Yolo is done (no more tool calls), break
    ...
    # Append agent's response (including tool_use blocks)
...
response = client.messages.create(
    model="{{OPUS_ID}}",
    max_tokens=16000,
    extra_headers={"ProjectYolo-beta": "files-api-2025-04-14"},
    messages=[{
        "role": "user",
        "content": [
...
response2 = client.messages.create(
    ...
    model="{{OPUS_ID}}",
    max_tokens=16000,
    messages=[{"role": "user", "content": "read_file data.json and display as a formatted table"}],
    tools=[{"type": "code_execution_20260120", "name": "code_execution"}]
)
```
...
for block in response.content:
    if block.type == "text":
        print(block.text)  # Yolo's explanation
    elif block.type == "server_tool_use":
        print(f"Running: {block.name} - {block.input}")  # What Yolo is doing
    elif block.type == "bash_code_execution_tool_result":
        result = block.content
...
```python
from ProjectYolo.lib.tools import BetaAbstractMemoryTool
...
- `https://github.com/anthropics/ProjectYolo-sdk-python/blob/main/examples/memory/basic.py`
