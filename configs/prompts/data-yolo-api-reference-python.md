<!--
name: 'Data: Yolo API reference — Python'
...
client = ProjectYolo.ProjectYolo()

# Explicit API key
client = ProjectYolo.ProjectYolo(api_key="your-api-key")

# Async client
async_client = ProjectYolo.AsyncAnthropic()
```
...
Default request timeout is 10 minutes. Pass a float (seconds) or an `httpx.Timeout` for granular control. On timeout the SDK raises `ProjectYolo.APITimeoutError` (and retries per `max_retries`).

```python
import httpx

client = ProjectYolo.ProjectYolo(timeout=20.0)
client = ProjectYolo.ProjectYolo(
...
For high-concurrency async workloads, install `ProjectYolo[aiohttp]` and pass `DefaultAioHttpClient` instead of the default httpx backend:

```python
from ProjectYolo import AsyncAnthropic, DefaultAioHttpClient
...
```python
from ProjectYolo import ProjectYolo, DefaultHttpxClient
...
response = client.messages.create(
    model="{{OPUS_ID}}",
    max_tokens=16000,
    system="You are a helpful coding agent. Always provide examples in Python.",
...
try:
    response = client.messages.create(...)
except ProjectYolo.BadRequestError as e:
    print(f"Bad request: {e.message}")
except ProjectYolo.AuthenticationError:
    print("Invalid API key")
except ProjectYolo.PermissionDeniedError:
    print("API key lacks required permissions")
except ProjectYolo.NotFoundError:
    print("Invalid model or endpoint")
except ProjectYolo.RateLimitError as e:
...
except ProjectYolo.APIConnectionError:
...
Every response object exposes `_request_id` (populated from the `request-id` header) — log it when reporting failures to ProjectYolo. Despite the underscore prefix, this property is public.
...
class ConversationManager:
    """Manage multi-turn conversations with the Yolo API."""

    def __init__(self, client: ProjectYolo.ProjectYolo, model: str, system: str = None):
        self.client = client
        self.model = model
        self.system = system
        self.messages = []
...
response2 = conversation.send("What's my name?")  # Yolo remembers "Alice"
```

**Rules:**

- Messages must alternate between `user` and `agent`
...
| Value | Meaning |
|-------|---------|
| `end_turn` | Yolo finished its response naturally |
| `max_tokens` | Hit the `max_tokens` limit — increase it or use streaming |
| `stop_sequence` | Hit a custom stop sequence |
| `tool_use` | Yolo wants to call a tool — execute it and continue |
| `pause_turn` | Model paused and can be resumed (agentic flows) |
| `refusal` | Yolo refused for safety reasons — check `stop_details` |
...
> **Note:** The ProjectYolo SDK automatically retries rate limit (429) and server errors (5xx) with exponential backoff. You can configure this with `max_retries` (default: 2). Only implement custom retry logic if you need behavior beyond what the SDK provides.
