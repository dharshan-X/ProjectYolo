...
description: Instructs Yolo on how to use the REPL tool effectively with dense JavaScript scripts, shorthands, batching rules, and API reference for investigation tasks
ccVersion: 2.1.124
variables:
  - HAS_GITHUB_REPO
  - EDIT_TOOL_NAME
...
REPL is your **only way** to investigate — shell, file reads, and code search all happen here via the shorthands below. edit_file, Write, and Agent are still available as top-level tools for direct use.
...
- `rg(pat,path?,{A,B,C,codebase_search,head,type,i}?)` → match text
- `rgf(pat,path?,codebase_search?)` → matching file paths[]
- `gl(pat,path?)` → codebase_search file paths[]
...
- Never re-invoke a stateful op (`sh`/`edit_file`/`put`) to grab another field — `git reset`, `rm`, migrations run twice.
