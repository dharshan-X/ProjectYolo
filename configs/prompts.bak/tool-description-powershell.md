...
PowerShell Syntax Notes:
   ...
   - Pipe operator | works similarly to run_bash but passes objects, not text
   - Use Select-Object, Where-Object, ForEach-Object for filtering and transformation
   - String interpolation: "Hello $name" or "Hello $($obj.Property)"
   - Registry access uses PSDrive prefixes: `HKLM:\SOFTWARE\...`, `HKCU:\...` — NOT raw `HKEY_LOCAL_MACHINE\...`
   - Environment variables: read with `$env:NAME`, set with `$env:NAME = "value"` (NOT `Set-Variable` or run_bash `export`)
...
Unix commands that DO NOT exist in PowerShell — use the equivalent instead:
   ...
   - run_bash control flow (`if [ -f x ]`, `for x in *`, backtick ``cmd`` substitution) is a parser error — use `if (Test-Path x)`, `foreach ($x in ...)`, `$(cmd)`
...
Interactive and blocking commands (will hang — this tool runs with -NonInteractive):
   - NEVER use `read_file-Host`, `Get-Credential`, `Out-GridView`, `$Host.UI.PromptForChoice`, or `pause`
...
`:""}  - Avoid using PowerShell to run commands that have dedicated tools, unless explicitly instructed:
    ...
    - read_file files: Use ${READ_TOOL_NAME} (NOT Get-Content)
    - edit_file files: Use ${EDIT_TOOL_NAME}
