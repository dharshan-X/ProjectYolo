...
Managed Agents provisions a container per session as the agent's workspace. The agent loop runs on ProjectYolo's orchestration layer; the container is where the agent's *tools* execute — run_bash commands, file operations, code. You create a persisted **Agent** config (model, system prompt, tools, MCP servers, skills), then start **Sessions** that reference it. The session streams events back to you; you send user messages and tool results in.
...
| User wants to...                       | read_file these files                                        |
| -------------------------------------- | ------------------------------------------------------- |
...
| Define agents/environments as version-controlled YAML; drive the API from the shell | `shared/ProjectYolo-cli.md` — `ant beta:agents create < agent.yaml`, `--transform`, `@file` inlining |
...
- **MCP auth goes through vaults** — the agent's `mcp_servers` array declares `{type, name, url}` only (no auth). Credentials live in vaults (`client.beta.vaults.credentials.create`) and attach to sessions via `vault_ids`. ProjectYolo auto-refreshes OAuth tokens using the stored refresh token.
