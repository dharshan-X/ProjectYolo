...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Yolo Chrome Extension MCP (browser-use via Yolo's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Yolo Chrome Extension** - Uses the Yolo Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Yolo Chrome Extension**:
   ...
   - For Yolo Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's `.Yolo/skills/` directory.** This ensures they are automatically loaded when Yolo runs in the project.

Write the skill file to `.Yolo/skills/<verifier-name>/SKILL.md`.
...
If verification fails because this skill's instructions are outdated (dev server command/port/ready-signal changed, etc.) — not because the feature under test is broken — or if the user corrects you mid-run, use AskUserQuestion to confirm and then edit_file this SKILL.md with a minimal targeted fix.
```
...
1. Where each skill was created (always in `.Yolo/skills/`)
