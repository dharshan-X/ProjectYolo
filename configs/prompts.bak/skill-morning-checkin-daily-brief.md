...
Check the local time against the start of their Catch-up hours from `Yolo.md` (default 9am if blank). If you're **more than two hours past work start**, end with a single line:
...
- **Mail** — unread from people or domains that matter (boss, reports, key collaborators — `Yolo.md` and `catch-up-state.json` priorities tell you who). Not a full inbox sweep — top 3-5 that actually need attention today.
