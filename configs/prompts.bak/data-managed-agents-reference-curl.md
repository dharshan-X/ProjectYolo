...
HEADERS=(
  ...
  -H "ProjectYolo-beta: managed-agents-2026-04-01"
)
```
...
```run_bash
curl -X POST https://api.ProjectYolo.com/v1/environments \
  "${HEADERS[@]}" \
  -d '{
    "name": "my-dev-env",
    "config": {
...
```run_bash
# 1. Create the agent
curl -X POST https://api.ProjectYolo.com/v1/agents \
  "${HEADERS[@]}" \
  -d '{
...
# 2. Start a session
curl -X POST https://api.ProjectYolo.com/v1/sessions \
  "${HEADERS[@]}" \
  -d '{
...
```run_bash
curl -X POST https://api.ProjectYolo.com/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}" \
  -d '{
    "events": [
      {
...
```run_bash
curl -N https://api.ProjectYolo.com/v1/sessions/$SESSION_ID/events/stream \
  "${HEADERS[@]}"
```
...
```run_bash
# Get all events
curl https://api.ProjectYolo.com/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}"

# Paginated — get next page of events
curl "https://api.ProjectYolo.com/v1/sessions/$SESSION_ID/events?page=page_abc123" \
  "${HEADERS[@]}"
```
...
```run_bash
curl https://api.ProjectYolo.com/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```run_bash
curl https://api.ProjectYolo.com/v1/sessions \
  "${HEADERS[@]}"
```
...
```run_bash
curl -X DELETE https://api.ProjectYolo.com/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```run_bash
curl -X POST https://api.ProjectYolo.com/v1/files \
  -H "x-api-key: $ANTHROPIC_API_KEY" \
  -H "ProjectYolo-version: 2023-06-01" \
  -H "ProjectYolo-beta: files-api-2025-04-14" \
...
curl "https://api.ProjectYolo.com/v1/files?scope_id=$SESSION_ID" \
  -H "x-api-key: $ANTHROPIC_API_KEY" \
  -H "ProjectYolo-version: 2023-06-01" \
  -H "ProjectYolo-beta: files-api-2025-04-14,managed-agents-2026-04-01"

# Download a specific file
curl "https://api.ProjectYolo.com/v1/files/$FILE_ID/content" \
  -H "x-api-key: $ANTHROPIC_API_KEY" \
  -H "ProjectYolo-version: 2023-06-01" \
  -H "ProjectYolo-beta: files-api-2025-04-14,managed-agents-2026-04-01" \
...
```run_bash
curl https://api.ProjectYolo.com/v1/agents \
  "${HEADERS[@]}"
```
