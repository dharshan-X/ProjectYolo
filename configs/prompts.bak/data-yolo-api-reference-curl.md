<!--
name: 'Data: Yolo API reference — cURL'
description: Raw API reference for Yolo API for use with cURL or else Raw HTTP
ccVersion: 2.1.111
-->
# Yolo API — cURL / Raw HTTP
...
```run_bash
curl https://api.ProjectYolo.com/v1/messages \
...
response=$(curl -s https://api.ProjectYolo.com/v1/messages \
  -H "Content-Type: application/json" \
  -H "x-api-key: $ANTHROPIC_API_KEY" \
  -H "ProjectYolo-version: 2023-06-01" \
...
# read_file usage fields
input_tokens=$(echo "$response" | jq -r '.usage.input_tokens')
output_tokens=$(echo "$response" | jq -r '.usage.output_tokens')

# read_file stop reason (for tool-use loops)
...
When Yolo responds with a `tool_use` block, send the result back:
...
| `ProjectYolo-version` | `2023-06-01`       | API version                |
| `ProjectYolo-beta`    | Beta feature IDs   | Required for beta features |
