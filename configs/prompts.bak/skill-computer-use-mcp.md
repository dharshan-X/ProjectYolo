...
- **Browsers** (Safari, Chrome, Firefox, Edge, Arc, etc.) → tier **"read"**: visible in screenshots, but clicks and typing are blocked. You can read what's already on screen. For navigation, clicking, or form-filling, use the Yolo-in-chrome MCP (tools named `mcp__claude-in-chrome__*`; load via ToolSearch if deferred).
