<!--
name: 'Data: Yolo API reference — Ruby'
...
> **Note:** The Ruby SDK supports the Yolo API. A tool runner is available in beta via `client.beta.messages.tool_runner()`. Agent SDK is not yet available for Ruby.

## Installation

```run_bash
gem install ProjectYolo
```
...
client = ProjectYolo::Client.new

# Explicit API key
client = ProjectYolo::Client.new(api_key: "your-api-key")
```
...
```ruby
class GetWeatherInput < ProjectYolo::BaseModel
  required :location, String, doc: "City and state, e.g. San Francisco, CA"
end

class GetWeather < ProjectYolo::BaseTool
...
rescue ProjectYolo::APIStatusError => e
